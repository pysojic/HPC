...
double PriceOptionJROpenMP(const OptionParams& params, size_t N) 
{
    double dt = params.T / N;
    double u = std::exp((params.r - 0.5 * params.vol * params.vol) * dt + params.vol * std::sqrt(dt));
    double d = std::exp((params.r - 0.5 * params.vol * params.vol) * dt - params.vol * std::sqrt(dt));
    double p = 0.5; // risk neutral probability
    double discount = std::exp(-params.r * dt);

    std::vector<double> optionValues(N + 1);

    // --- Compute Terminal Payoffs in Parallel ---
    #pragma omp parallel for schedule(static)
    for (int j = 0; j <= static_cast<int>(N); j++) 
    {
        double S_val = params.S * std::pow(u, j) * std::pow(d, N - j);
        if (params.OptionType) {
            optionValues[j] = std::max(S_val - params.K, 0.0);
        } else {
            optionValues[j] = std::max(params.K - S_val, 0.0);
        }
    }
...
int main()
{
    ...
    double price = PriceOptionJR(option6, N);
    auto t2 = std::chrono::high_resolution_clock::now();

    std::cout << "Elapsed time: " <<
    std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() << " microseconds\n";
    std::cout << "Option (call) price (multi-threaded): " << price << std::endl;
